...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Package Body XXVEN_AR_INTERF_PROGRAMARE_PK
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE BODY "APPS"."XXVEN_AR_INTERF_PROGRAMARE_PK" AS
-- +=================================================================+
...
--
   --
   ----------------------------------------------------------------------
  -- L_DEBUG VARCHAR2(2) := NVL(FND_PROFILE.VALUE('XXVEN OM DEBUG PEDIDOS'),'N'); --'S';
  --
    ...
    LOOP
      ...
      BEGIN
        ...
        LOOP
          ...
          END;
          --
          -- Validacao unidade medida
          /*
          BEGIN
            l_mens_erro := ' Erro ao selecionar informacoes sobre unidade de medida para o item (tabela de conversoes mtl_uom_conversions): ' || r_ped_line_prog.unidade_medida;
            SELECT unit_of_measure
              INTO l_uom_code_line
              FROM mtl_uom_conversions
             WHERE inventory_item_id = l_inventory_item_id_line
               AND NOT EXISTS (SELECT b.unit_of_measure
                                 FROM mtl_units_of_measure b
                                WHERE b.unit_of_measure = mtl_uom_conversions.unit_of_measure
                                  AND b.base_uom_flag = 'Y');
          EXCEPTION
            WHEN OTHERS THEN
               l_mens_erro     := l_mens_erro || '. ' || SUBSTR(SQLERRM,1,150);
               l_uom_code_line := r_ped_line_prog.unidade_medida;
               P_DEBUG(l_mens_erro);
          END;
          */
          --
          ...
          IF NVL(r_ped_line_prog.valor_desconto,0) = 0 THEN
             ...
             -- Movimenta Material
             --
             -- A movimentacao de estoque para a nota acima gerada (insert ra_interface_lines_all) acontecera na trigger denominada XXVEN_RETORNO_PROCFIT_NFE para a tabela jl_br_customer_trx_exts
             -- A movimentacao de material sera chamada pela package/procedure "xxven_ebs_util_pk.prc_inv_movimenta_material_p".
             -- Apenas movimentara estoque quando a nota acima gerada for status=2 na tab jl_br_customer_trx_exts, ou seja, finaliza com sucesso pela SEFAZ
             --
             l_mens_erro := NULL;
             /*
             movimenta_estoque( p_inventory_item_id  => l_inventory_item_id_line
                              , p_qtde               => r_ped_line_prog.quantidade
                              , p_uom                => l_uom_code_line
                              , p_organization_id    => l_sold_from_organization_id
                              , p_cost_of_sales_acc  => l_cost_of_sales_acc 
                              , x_msg_error          => l_mens_erro) ;
             */
             --
             IF l_mens_erro IS NOT NULL THEN
                P_DEBUG(l_mens_erro);
                -- Ha mensagem de erro porem apenas display dentro da desta proc. movimenta_estoque
                -- Caso haja erro na proc movimenta_estoque, o procedimento prossegue. NAO é impeditivo para continuar o processamento do pedido
                NULL;
             END IF;
             --
          END IF; 
